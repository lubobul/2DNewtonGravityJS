...
export class SimulationEngine {
    ...
    private distanceDivisor: number = 1000000;
    ...
    private drawTraces(): void {
        ...
        for (let i = 0, len = this.gravityEngine.objectsTrajectoriesStack.length; i < len; i++)
        {
            ...
            {
                this.canvas.drawPixel(trace[j].x, trace[j].y);
            }
        }
    }
...
//Inserts a UI selected object into the simulation, calculating direction ad speed proportional to the created drag & drop vector
    private insertNewObjectIntoTheSimulation(): void{
        ...
        let cartesianCoordinates = cartesianToPolar(this.object_vector_direction_x -this. object_vector_start_x, this.object_vector_direction_y - this.object_vector_start_y);

        let v = (cartesianCoordinates.r * this.new_object_velocity_factor); //linear progression of velocity based on vector length (needs to be figured out properly)
